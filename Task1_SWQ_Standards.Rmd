...
ggsave(here(plotsave, "PlotB_tmax_by_day_year_site_rolling3.png"),
          height = 7, width = 9, dpi = 400)
```
...
save_kable(day_table,
           here(plotsave, "TableA_tmax_n_table3.html"))
```
...
ggsave(here(plotsave,
  "PlotF_dmin_by_day_year_24h_avg2.png"),
  height = 7, width = 9, dpi = 400)
```
...
ggsave(here(plotsave, "PlotG_days_below_DO_threshold2.png"),
       height = 7, width = 9, dpi = 400)
```
...
ggsave(here(plotsave, 
  "PlotH_days_below_DO_threshold_boxplot2.png"),
       width = 8,
       height = 6, dpi = 400)
```
...
ggsave(
  "output/Task1_SWQS_loggers/PlotJ_days_exceeding_any_threshold_boxplot.png",
       ...
       height = 6, dpi = 400)
```
# Model A: estimating proportion of summer days exceeding TM temperature standards
Bayesian hierarchical model. Code adapted from the random effects binomial model described in Kery & Royle (2015) book: Applied Hierarchical Modeling. Includes Site x Year random effects.
```{r}
...
saveRDS(
  jags_mod4,
        here(plotsave,
             "statistical_analysis/model_output/JAGS_TMtemp_model.rds")
  )
...
```
# Plot Q: Plots of temperature over time for each site (panels by year)
```{r}
...
```
# Plot R: Plots of DO over time for each site (panels by year)
```{r}
