...
gam_1 <- gam(max_temp ~ s(day) + yearf,
             data = tmax_gam,
             family = gaussian)
...
jags_data2 <- list(n_total = nrow(tstatJAGS),
                  n_years = length(unique(tstatJAGS$yearf)),
                  n_sites = length(unique(tstatJAGS$site)),
                  TMex = TMex_mat,
                  TMex_n = TMex_n_mat,
...
cat(file = "scripts/test_mod.txt",
"
model{

for(i in 1:n_total){
  TMex[i] ~ dbern(lp[year[i])
  lp[year[i]] ~ dnorm(site_mu, site_tau)
}

site_mu ~ dnorm(0, 0.001) # prior after Kery 2010 (p234)
site_tau <– 1 / (site_sigma * site_sigma)
site_sigma ~ dunif(0, 10)
}
")

cat(file = "scripts/test_mod_kery.txt",
"
model {
# Priors
for (i in 1:n_years){
lp[i] ~ dnorm(mu.int, tau.int) # Intercepts
}
...
# attempt adding another level (site) onto this
cat(file = "scripts/test_mod_kery2.txt",
"
model {
...
# another attempt at site & year levels (this time with the orig data)
cat(file = "scripts/test_mod_kery3.txt",
"
model {
# Priors
for(i in 1:n_sites){
  for (j in 1:n_years){
lp[i,j] ~ dnorm(mu.alpha[i], tau.alpha) # Intercepts
}
}
mu.int ~ dnorm(mu.int2, tau.int2)
tau.int ~ dgamma(0.001, 0.001)

mu.int2 ~ dnorm(0, 0.001) # Hyperparameter for random intercepts
tau.int2 <- 1 / (sigma.int * sigma.int)
sigma.int2 ~ dunif(0, 10)
...
jags_mod <- jagsUI::jags(data = jags_data,
             model.file = "scripts/test_mod_kery.txt",
             n.iter = 2000, n.burnin = 1000,
             parameters.to.save = c("lp", "realp",
                                    "mu.int", "sigma.int"),
             n.chains = 3, parallel = T, n.thin = 1
             )

jags_mod2 <- jagsUI::jags(data = jags_data2,
             model.file = "scripts/test_mod_kery2.txt",
             n.iter = 2000, n.burnin = 1000,
             parameters.to.save = c("lp", "mu.sites", "tau.sites",
                                    "mu.year", "tau.year",
                                    "mu.river", "tau.river"),
             n.chains = 3, parallel = T, n.thin = 1
             )
...
# another try with Kery AHM book
...
jags_data3 <- list(n_total = nrow(tstatJAGS),
                  n_years = length(unique(tstatJAGS$yearf)),
                  n_sites = length(unique(tstatJAGS$site)),
                  TMex = TMex_mat,
                  TMex_n = TMex_n_mat,
...
logit(theta[i,j]) <- alpha0.site[i] + beta[1]*y1[i,2] +
                        beta[2]*y2[i,j] + beta[3]*y3[i,j] +
                      beta[4]*y4[i,j] + beta[5]*y5[i,j] +
                      beta[6]*y6[i,j]
...
jags_mod3.1 <- jagsUI::jags(data = jags_data3,
             model.file = "scripts/AHM_mod.txt",
             n.iter = 2000, n.burnin = 1000,
...
mod3sum <- jags_mod3.1$summary %>%
  as.data.frame() %>%
  select(mean, q2.5 = 3, q97.5 = 7) %>%
  mutate(parameter = row.names(jags_mod3.1$summary)) %>%
...
mod3_siteavg <- jags_mod3.1$summary %>%
  as.data.frame() %>%
  select(mean, q2.5 = 3, q97.5 = 7) %>%
  mutate(parameter = row.names(jags_mod3.1$summary)) %>%
  filter(grepl(parameter, pattern = "alpha")) %>%
  slice(1:18) %>%
...
ggplot(mod3sum) +
  ...
  geom_hline(data = mod3_siteavg, aes(yintercept = mean), 
              color = "red") +
...
ggsave("test.png", width = 7, height = 5, dpi = 400)

# to do
# ADD IN THE RAW PROPORTIONS TO VIEW HOW IT WORKED
# CENSOR YEARS WITH NO OBSERVATIONS
# ADD IN MEAN SITE LINE
# ADD IN MEAN YEAR SYMBOL?

# make plot of just overall estimated site means +/- CI
# estimate derived "slope" by year in JAGS where possible



```

# another try with Kery AHM book - SitexYear effects
...
jags_mod4 <- jagsUI::jags(data = jags_data4,
             ...
             parameters.to.save = c("theta", "alpha0.site",
                                    ...
                                    "tau.year.effect",
                                    "river.mean"),
             n.chains = 3, parallel = T, n.thin = 20
             )
...
mod4sum <- jags_mod4$summary %>%
  ...
  mutate(order = case_when(order == "r " ~ 85,
         ...
         samp_size = case_when(is.na(TMex_sum)==T ~ "",
                               TRUE ~ 
                                 paste0(as.character(TMex_sum), 
                                        " / ", 
                                        as.character(TMex_n)))
...
ggplot(mod4sum) +
  ...
  scale_y_continuous(limits = c(0,1),
                     breaks = seq(0,1,by = .5)) +
  labs(y = "Proportion of days exceeding \nTrout Maintenance temperature SWQS (Jun-Aug)")

ggsave("output/Task1_SWQS_loggers/statistical_analysis/trends.png", width = 7, height = 5, dpi = 400)
