...
# saveRDS(
#   jags_mod4,
#         "output/Task1_SWQS_loggers/statistical_analysis/JAGS_TMtemp_model.rds"
#   )

# to read in the model results instead of running it
jags_mod4 <- 
  readRDS("output/Task1_SWQS_loggers/statistical_analysis/JAGS_TMtemp_model.rds")
...
ggsave("output/Task1_SWQS_loggers/statistical_analysis/trends_tempTM.png", width = 7, height = 5, dpi = 400)
```
...
ggsave("output/Task1_SWQS_loggers/statistical_analysis/standardized_site_comparisons_tempTM.png", width = 6, height = 7, dpi = 400)
```
...
# saveRDS(
#   jags_mod_DO,
#         "output/Task1_SWQS_loggers/statistical_analysis/JAGS_TM_DO_model.rds"
#   )

# to read in the model results instead of running it
jags_mod_DO <- 
  readRDS("output/Task1_SWQS_loggers/statistical_analysis/JAGS_TM_DO_model.rds")
...
ggsave("output/Task1_SWQS_loggers/statistical_analysis/trends_DO_TM.png", width = 7, height = 5, dpi = 400)
```
# Plot estimated proportion of days exceeding TM temp standards by site, standardizing to 2018 conditions
```{r}

y18 <- jags_mod_DO$summary %>%
  as.data.frame() %>%
  select(mean, q2.5 = 3, q97.5 = 7) %>%
  mutate(parameter = row.names(jags_mod_DO$summary)) %>%
  slice(37:54) %>%
  mutate(site = unique(dostat$site)) %>%
  mutate(site = fct_reorder(site, mean))
...
x11(6,7); y18 %>%
  ...
  geom_point(aes(x = site, y = mean), 
             size = 4, color = "firebrick") +
  coord_flip() +
  labs(y = "Predicted proportion of days (Jun-Aug) exceeding \nTrout Maintenance dissolved oxygen thresholds \n(standardized to 2018 conditions)", x = "") +
  theme_bw() +
  theme(text = element_text(size = 14))

ggsave("output/Task1_SWQS_loggers/statistical_analysis/standardized_site_comparisons_DO_TM.png", width = 6, height = 7, dpi = 400)
```
