...
```
# Plot A: Temperature SWQS: plot daily max temp for each site/year vs. thresholds
```{r}
...
ggsave("output/Task1_SWQS_loggers/PlotA_tmax_by_day_year_site.png",
        height = 7, width = 9, dpi = 400)
```
#Plot B: Temperature SWQS: plot rolling-average daily max temp for each site/year vs. thresholds
```{r}
...
```
# Plot C: number of days exceeding temperature thresholds
```{r}
...
rm(temp_count_bad)
```
## Plot C (individual): no. days exceeding temp. thresholds - individually by site
I.e., Plot C but as one plot per site
```{r}
...
ggsave(
  paste0("output/Task1_SWQS_loggers/PlotC_individual/",
...
```
## Plot G: number of days below DO thresholds (time series)
To change font size of years, change the number in this line of code: "axis.text.x = element_text(size = 8),"
```{r}
...
#        height = 7, width = 9, dpi = 400)
```
## Plot no. days below DO thresholds - individually by site
I.e., Plot G but as one plot per site
```{r}
...
ggsave(
  paste0("output/Task1_SWQS_loggers/PlotG_individual/",
...
jags_mod4 <- 
  readRDS("output/Task1_SWQS_loggers/statistical_analysis/hierarchical_model_output_files/JAGS_TMtemp_model.rds")
...
# ggsave("output/Task1_SWQS_loggers/PlotK_temp_TM_exceed_hierarchical_model_trends.png", width = 7, height = 5, dpi = 400)
```
...
# to read in the model results instead of running it
jags_mod_DO <- 
  readRDS("output/Task1_SWQS_loggers/statistical_analysis/hierarchical_model_output_files/JAGS_TM_DO_model.rds")
...
# ggsave("output/Task1_SWQS_loggers/PlotL_DO_TM_exceed_hierarchical_model_trends.png", width = 7, height = 5, dpi = 400)
```
...
# ggsave("output/Task1_SWQS_loggers/PlotM_temp_TM_exceed_hierarchical_model_standardized_site_comparisons.png", width = 6, height = 7, dpi = 400)
```
# Table of hierarchical model results - temperature
```{r}
...
save_kable(model_table_temp,
           "output/Task1_SWQS_loggers/TableE_modeled_p_TM_temp.html")

```
# Table of hierarchical model results - DO
```{r}
...
save_kable(model_table_DO,
           "output/Task1_SWQS_loggers/TableF_modeled_p_TM_DO.html")
