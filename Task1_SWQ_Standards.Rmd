...
sink(paste0("output/Task1_SWQS_loggers/",
              "statistical_analysis/GAM_results.txt"))
summary(gam_1)
sink()
...
tmax_jul15 %>%
  ...
  theme(text = element_text(size = 14),
        strip.background = element_rect(fill = "white")) +
  labs(x = "Predicted maximum temperature on July 15th", 
       y = "",
...
tmax_hat = predict(gam_1, 
                     newdata = 
                       expand.grid(site = "TNC_2", 
                                   yearf = 2016:2021, 
                                   day = 91:288), se.fit = T)
plot_data <- expand.grid(site = "TNC_2",
                         year = 2016:2021, 
                                   day = 91:288) %>%
...
plot_data %>%
  arrange(site, year, day) %>%
ggplot() +
  # geom_ribbon(aes(x = day, ymin = tmax_LL, ymax = tmax_UL, fill = yearf)) +    
  geom_point(aes(x = day, y = max_temp), color = "darkgray",
               alpha = 0.5) +

  geom_line(aes(x = day, y = tmax_hat, color = yearf)) +
  facet_wrap(~yearf)
```
