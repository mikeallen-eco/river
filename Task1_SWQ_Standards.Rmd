...
To do:
1. graphs of individual sites w/ panels by year (maybe raw & rolling average on same graph)
2. add TNC_80's graphs
...
dmin <- p %>%
  ...
  mutate(day = yday(datetime)) %>%
  group_by(site, year, day) %>%
  summarise(min_DO = min(do),
            mean24_DO = mean(do),
            min_DOadj = min(do_adj),
            mean24_DOadj = mean(do_adj),
            .groups = "drop") %>%
  droplevels() %>%
  right_join(expand.grid(site = unique(p$site), 
                         year = 2016:2021, day = 82:339)) %>%
...
DO_count <- dmin %>%
  ...
  select(site, year, Threshold, days_under) %>%
  mutate(Threshold = gsub(gsub(Threshold, pattern = "_", 
                          replacement = " "), pattern = "\\.",
                          replacement = "-"))

# summarize number of days below DO thresholds (adjusted DO)
DO_count_adj <- dmin %>%
  filter(is.na(min_DOadj)==F) %>%
  mutate(min_7 = ifelse(min_DOadj < 7, 1, 0),
         min_5 = ifelse(min_DOadj < 5, 1, 0),
         min_4 = ifelse(min_DOadj < 4, 1, 0),
         mean24_6 = ifelse(mean24_DOadj < 6, 1, 0),
         mean24_5 = ifelse(mean24_DOadj < 5, 1, 0),
         FW2_TP = case_when(min_7 > 0 ~ 1,
                            TRUE ~ 0),
         FW2_TM = case_when(min_5 > 0 ~ 1,
                            mean24_6 > 0 ~ 1,
...
# ggsave("output/Task1_SWQS_loggers/PlotB_tmax_by_day_year_site_rolling.png",
# height = 7, width = 9, dpi = 400)
```
...
# save_kable(day_table, "output/Task1_SWQS_loggers/TableA_tmax_n_table.html")
...
# ggsave("output/Task1_SWQS_loggers/PlotC_days_above_temp_threshold1.png",
#        height = 7, width = 9, dpi = 400)
```
...
# save the table to HTML file
# save_kable(days_over_table, "output/Task1_SWQS_loggers/TableB_tmax_days_over_TM_table.html")
...
# ggsave("output/Task1_SWQS_loggers/PlotD_days_above_temp_threshold.png", 
#        width = 8,
#        height = 6, dpi = 400)
```
