...
rm(p)
```
# Plot A: Daily mean temperature with monthly averages by year (separate plots by site)
...
```
# Plot B: monthy mean temperatures by year (separate plots for each site)
```{r}
sites <- unique(tdata$site)
...
```
# Plot D: Daily mean DO with monthly averages by year (separate plots by site)
Error bars are the daily min/max. Black circles represent monthly averages of the daily mean. 
```{r}
sites <- unique(tdata$site)
...
```
# Plot E: mean/min/max temperature during migratory fish periods
...
```
# Plot F: mean/min/max DO during migratory fish periods
Only for sites accessible by migratory fish.
```{r}
# Create list of sites (excluding those not accessible)
sites <- unique(dodata$site)
...
rm(plot_data, annotate_df, max_y)
```
# Table A: average daily temperatures by month
...
```
# Table B: average daily temperatures for 1 Jul - 15 Sept
...
```
# Table C: average daily temperatures for migratory fish seasons
...
# fit model during spring migration period
...
```
# Plot G: modeled mean temp by fish stage w/ CIs
```{r}
...
(fall_temp_plot <- 
  ...
  geom_errorbar(aes(x = yearn, ymin = mean-2*se, ymax = mean+2*se),
                ...
                data = filter(season_pts, season == "fall_mig")) +
  theme_bw() +
  scale_y_continuous(limits = c(10, 20.5)) +
    scale_x_continuous(breaks = 1:6,
                       labels = 2016:2021) +
    labs(x = "", 
         y = "Mean temperature trend (C)",
