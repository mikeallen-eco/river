...
---
# To Do
1. Plots E&F: make points bigger
2. Plots E&F: represent as mean +/- 2*SE
3. Plots E&F: add min/max language to x-axis title
...
```{r}
# intercept varying among sites and among days within sites (nested random effects)
...
# fit model during spawning period
temp_trend_spawn <- 
  lmerTest::lmer(temp ~ yearn + (1 | site/dayf), 
             data = filter(tdata, is.na(temp)==F,
                           season == "spawn"))
...
(spawn_temp_plot <- 
    ggplot(spawn_temp_trend_df, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), 
              alpha = .1) +
  theme_bw() +
...
(larvae_temp_plot <- 
    ggplot(larvae_temp_trend_df, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), 
              alpha = .1) +
  theme_bw() +
...
(juv_temp_plot <- 
    ggplot(juv_temp_trend_df, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), 
              alpha = .1) +
  theme_bw() +
...
(fall_temp_plot <- 
    ggplot(fall_temp_trend_df, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), 
              alpha = .1) +
  theme_bw() +
