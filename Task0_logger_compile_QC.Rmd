...
1. see 3 QC filter steps for hobo (see section 0.1)
...
# compile and add in 2022 hobo data if it exists
if(length(list.files(here(logger_path, "2022 data", "HOBO_2022")))!=0){
hobo <- hobo %>%
...
# compile and add in 2023 hobo data if it exists
if(length(list.files(here(logger_path, "2023 data", "HOBO_2023")))!=0){
hobo <- hobo %>%
...
# compile and add in 2024 hobo data if it exists
if(length(list.files(here(logger_path, "2024 data", "HOBO_2024")))!=0){
hobo <- hobo %>%
...
# compile and add in 2025 hobo data if it exists
if(length(list.files(here(logger_path, "2025 data", "HOBO_2025")))!=0){
hobo <- hobo %>%
  bind_rows(logger_compile("2025", type = "hobo")) %>%
  filter(is.na(year) != TRUE,
         temp != -9999.00,
         temp != -888.88)
}
...
```
# 0.7. Make final hobo-hydro file with temp and DO
```{r}
p <- hobo %>%
...
```
# 0.8. plot temperature time series for each site/year vs. thresholds (showing loggers)
```{r}
...
```
# 0.9. plot DO time series for each site/year vs. thresholds (showing loggers)
```{r}
...
```
# 0.10. plot DO (adjusted) time series for each site/year vs. thresholds (showing loggers)
```{r}
