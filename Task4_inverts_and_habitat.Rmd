...
#PlotA - data visualization of index components
Scatterplot matrix
```{r}
...
my_smooth_fn <- function(data, mapping, ...){
  p <- ggplot(data = data, mapping = mapping, color = tnc_code) + 
    geom_point(alpha = 0.5, shape = 1, size = 1) + #color = "steelblue", 
...
g
```
# Dot plot of overall index by site/year
```{r}
...
ggsave(here(task_path, 
            "PlotB_mean_macroinvertebrate_index_by_site.png"),
