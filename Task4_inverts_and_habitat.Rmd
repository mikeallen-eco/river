...
inv %>%
  ...
  geom_point(aes(x = year, y = index, color = tnc_code), 
            size = 4) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_color_manual(values = c("olivedrab", "orange", "dodgerblue")) +
    theme_bw() +
    theme(text = element_text(size = 14)) +
    labs(y = "Macroinvertebrate Index", x = "",
...
ggsave(here(task_path,
            "PlotG_mean_habitat_score_by_site2.png"),
...
ggsave(here(task_path,
            "PlotH_habitat_score_vs_macro_by_site2.png"),
...
botline <- data.frame(type = c("Habitat","Habitat", 
                     ...
                     year = rep(c(min(plot_data$year),
                                  max(plot_data$year)),2),
                     index = c(55/2,55/2, 21, 21)
  )

ggplot() +
  # top threshold line
  geom_line(aes(x = year, y = index), lty = 2, 
            color = color_vector_inv[1],
             alpha = 0.75,
            data = topline) +
  # mid threshold line
  geom_line(aes(x = year, y = index), lty = 2, 
            color = c(color_vector_inv[2], 
                      color_vector_inv[2],
                      color_vector_inv[2],
                      color_vector_inv[2]),
             alpha = 0.75,
  ...
  geom_line(aes(x = year, y = index), lty = 2, 
            color = c(color_vector_inv[3]),
             alpha = 0.75,
            data = botline) +
  geom_hline(yintercept = 0, color = color_vector_inv[4], 
             lty = 2, alpha = 0.75) +
  geom_line(aes(x = year, y = index), 
            color = "darkgray",
            size = 1,
  ...
  scale_color_manual(values = c(color_vector_inv, color_vector_inv),
                     drop = F) +
  theme(text = element_text(size = 14),
...
```
# Plot M - Change in macro index over time (all sites) 
```{r}
inv_mean <- inv %>%
  group_by(tnc_code, year) %>%
...
ggplot(inv_mean) +
  geom_errorbar(aes(x = year, ymin = index_mean-2*index_se,
                    ymax = index_mean+2*index_se),
                width = 0, size = 1) +
    geom_point(aes(x = year, y = index_mean), 
             size = 4, shape = 21, fill = "steelblue") +
  geom_text(aes(x = year, y = 1, label = n),
            color = "darkgray") +
  annotate(geom = "text", x = 2016, y = 7, 
           label = "No. locations:",
           color = "darkgray",
           hjust = .1) +
  labs(x = "", y = "Macroinvertebrate index\n(+/- 2 SE)") +
  theme_bw() +
  theme(text = element_text(size = 14))
