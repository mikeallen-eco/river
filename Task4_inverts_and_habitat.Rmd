...
png(here(task_path, 
         ...
         paste0("PlotC_", the_title, "_habitat_radar.png")), 
    res = 400,
    height = 5, width = 5, units = "in")

# data
data <- hab_radar[i,2:14]

# add 2 lines to the dataframe: the max and min you want to show on the plot
data <- rbind(rep(20,13) , rep(0,13) , data)
...
color_vector <- c("olivedrab",
            wes_palettes$BottleRocket2[2:1])

all_colors <- data.frame(
    habitat_rating = c("Optimal", "Marginal", "Sub-Optimal"),
    col = color_vector)

the_colors = hab %>%
  filter(tnc_code == i) %>%
  select(habitat_rating) %>%
  distinct() %>%
  arrange(habitat_rating) %>%
    left_join(all_colors, by = "habitat_rating")
...
hab %>%
  ...
  geom_point(aes(x = year, y = habitat_score, 
            ...
            size = 4, data = filter(hab, tnc_code == i)) +
  ...
  ggsave(here(task_path, "PlotK_habitat_score_by_year",
              paste0("PlotD_hab_score_by_year_site_", i, ".png")),
         width = 6, height = 4, dpi = 400)
}
