...
# Tree planting - evaluating temperature trend across years
```{r}
treet_jags <- tree %>% 
  filter(is.na(d8.4) == F,
         monthf %in% 5:9) %>%
...
model{
  ...
  # likelihood
  for( i in 1:N ){
  tdiff[i] ~ dnorm(mu[i], tau)
  mu[i] <- beta0[month[i]] + beta.year[month[i]]*year[i]
  }
  
  # hyper priors
  for(i in 5:9){
  beta0[i] ~ dnorm(mu.beta0, tau.beta0)
  beta.year[i] ~ dnorm(mu.beta.year, tau.beta.year)
  }
  ...
  tau.beta0 ~ dgamma(0.001, 0.001)
  tau.beta.year ~ dgamma(0.001, 0.001)

  # derrived
  for(i in 1:5){
  diff_pred[i] <- mu.beta0 + mu.beta.year*i
  }
  
}
"
)
test <- lme4::lmer(tdiff)
treet_jags_mod <- jagsUI::jags(data = treet_jags_data,
             ...
             parameters.to.save = c("mu.beta0",
                                    ...
                                    "tau.beta.year",
                                    "diff_pred"),
             n.chains = 3, parallel = T, n.thin = 6
             ); treet_jags_mod
