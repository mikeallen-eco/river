...
model{
  ...
  # derrived
  for(i in 1:5){
  diff_pred[i] <- mu.beta.year*(i-year_mean)
  }
...
treet_jags_plot <- apply(.495+treet_jags_mod$sims.list$diff_pred, 2, function(x) quantile(x, c(0.025, 0.1, 0.5, 0.90, 0.975))) %>%
  t() %>%
  as.data.frame() %>%
...
tree %>%
  ...
  scale_color_manual(values = wes_palettes$Darjeeling1) +
 scale_y_continuous(limits = c(-0.5, 1.5)) +
  annotate("text", x = 5.25, y = -0.49, 
           label = "mean trend = +0.04 C/yr\n95% CI = -0.003, 0.088", 
           hjust = 1, vjust = 0)
...
```
# Model D - Tree planting temperature trend (within - upstream)
...
```
# Plotting tree planting temperature trend (within - upstream)
TNC_5 - TNC_4
Model to estimate mean trend. Redo plot J showing trend info.
```{r}
treet_jags_mod <- here(task_path,
             "statistical_analysis/",
             "ModelD_tree_planting_temp_trends_within_up.rds")

treet_jags_plot <- apply(.386+treet_jags_mod2$sims.list$diff_pred, 2, function(x) quantile(x, c(0.025, 0.1, 0.5, 0.90, 0.975))) %>%
  t() %>%
  as.data.frame() %>%
  rename(q2.5 = 1, q10 = 2, med = 3, q90 = 4, q97.5 = 5) %>%
...
tree %>%
  ...
  scale_color_manual(values = wes_palettes$Darjeeling1) +
  scale_y_continuous(limits = c(-0.5, 1.5)) +
  annotate("text", x = 4.25, y = -0.49, 
           label = "mean trend = +0.044 C/yr\n95% CI = -0.027, 0.116", 
           hjust = 1, vjust = 0)
  
# ggsave(here::here("output/Task3_project_impacts/",
#                 "PlotJ2_tree_planting_upstream_within_diff_by_mo_year_w_trend.png"),
#                   height = 5, width = 7, dpi = 400)

```


# Plot L - Tree planting - upstream/downstream DO diff by month/year
...
# ggsave(here::here("output/Task3_project_impacts/",
#                   "PlotL_tree_planting_up_downstream_DO_diff_by_mo_year.png"),
#                   height = 5, width = 7, dpi = 400)
```
# Plot M - Tree planting - upstream/within DO diff by month/year
```{r}
treedo %>%
  filter(yearf %in% 2018:2021,
         monthf %in% 5:9) %>%
...
# ggsave(here::here("output/Task3_project_impacts/",
#                 "PlotM_tree_planting_upstream_within_DOdiff_by_mo_year.png"),
#                   height = 5, width = 7, dpi = 400)
```
# Plot N - Columbia Dam - Turbidity upstream vs. downstream - raw data by year
upstream: TNC_23, downstream: TNC_25 (& 24), in: 80 & 81
first notch July 2018, completed April 2019
```{r}
colturb %>%
...
```
# Plot O - Columbia Dam - log turbidity difference, upstream vs. downstream - raw data by year
upstream: TNC_23, downstream: TNC_25 (& 24), in: 80 & 81
first notch July 2018, completed April 2019
```{r}
colturb %>%
...
```
# PlotP_assessing variability in upstream/downstream turbidity by month/year
```{r}
colturb %>%
  ggplot() +
  geom_abline(intercept = 0, slope = 0, lty = 3, color = "steelblue")  +
...
#                   height = 5, width = 7, dpi = 400)
```
# Model E - Columbia Dam Turbidity - downstream/upstream BACI - by stage
Upstream: TNC_23
Downstream: TNC_25
```{r}
...
```
# Plot Q estimated upstream/downstream tdiff by restoration stage
```{r}
...
```
# Plot R - Paulina Dam - Temperature
TNC_18 and TNC_19 (compare with TNC_13?)
```{r}
...
```
# Plot S - Paulina downstream (TNC_19) - upstream (TNC_18)
Temp, DO, & Turbidity
```{r}
...
```
# Plot T - Hyper Humus - Temperature & DO
TNC_2 (compared with other site means)
```{r}
# summarize daily temp data by month
tsum_hyper <-  tdata %>%
    group_by(site, yearf, month, day, dayf) %>%
...
```
# Plot U - County Line Dam - Temperature & DO
...
```
# Plot V - Tree planting area - Temperature & DO
...
```
# Plot W - Columbia Dam - Temperature & DO
Monthly averages each year for TNC_4, 5, and 8 (compared with other site means)
```{r}
...
```
# Plot X - Columbia Dam - Temp. upstream vs. downstream - raw data by year
upstream: TNC_23, downstream: TNC_25 (& 24), in: 80 & 81
first notch July 2018, completed April 2019
```{r}
col %>%
...
```
# Plot Y - Columbia Dam - July Temp. upstream vs. downstream - raw data by year
upstream: TNC_23, downstream: TNC_25 (& 24), in: 80 & 81
first notch July 2018, completed April 2019
```{r}
col %>%
...
```
# Plot Z - Columbia Dam - DO upstream vs. downstream - daily data by year
upstream: TNC_23, downstream: TNC_25 (& 24), in: 80 & 81
first notch July 2018, completed April 2019
```{r}
coldo %>%
...
```
# Plot ZA - Columbia Dam - DO upstream vs. downstream - daily data by year - JULY data only
upstream: TNC_23, downstream: TNC_25 (& 24), in: 80 & 81
first notch July 2018, completed April 2019
```{r}
coldo %>%
...
```
# Plot ZB - Columbia Dam - monthly mean temperature w/ envelope
TNC_25, 23, 80, 81
```{r}
# summarize daily temp data by month
tsum_col <-  tdata %>%
...
summary(filter(monthly_diff, yearf %in% 2019:2021)$diff)

ggsave(here::here("output/Task3_project_impacts/",
                  "PlotZB_columbia_dam_upstream_ds_temp5.png"),
                  height = 4, width = 6.5, dpi = 400)
```
# Plot ZC - Columbia downstream (TNC_25) - upstream (TNC_23)
Temp, DO, & Turbidity
```{r}
